...
class _EmployeeDetailScreenState extends State<EmployeeDetailScreen> {
  ...
  Widget _buildEmployeeContent() {
    ...
    // --- ✨ DYNAMIC LOCATIONS LOGIC (JOIN ALL NAMES) ---
    String locationName = "General";
    var locData = employeeData!['locations'];

    if (locData is List && locData.isNotEmpty) {
      // Har element se name nikal kar join kar rahe hain
      List<String> names = locData.map((loc) {
        if (loc is Map) return loc['name']?.toString() ?? "";
        return loc.toString();
      }).where((s) => s.isNotEmpty).toList();

      if (names.isNotEmpty) {
        locationName = names.join(", "); // Result: "Location1, Location2"
      }
    } else if (locData is Map) {
      locationName = locData['name']?.toString() ?? "General";
    } else if (locData != null && locData.toString().isNotEmpty) {
      locationName = locData.toString();
    }
              ...
              child: ClipOval(
                child: SizedBox(
                  ...
                  child: (imageUrl != null && imageUrl.isNotEmpty)
                    ...
                    imageUrl,
                    fit: BoxFit.cover,
                    errorBuilder: (context, error, stackTrace) => Image.asset('assets/img.png', fit: BoxFit.cover),
...
//
//     if (locData is List && locData.isNotEmpty) {
//       // If it's a List, check if the first item is a Map
//       var firstLoc = locData[0];
//       var secondLoc = locData[1];
//       if (firstLoc is Map && secondLoc is Map) {
//         locationName = firstLoc['name']?.toString() ?? "General";
//         locationName2 = secondLoc['name']?.toString() ?? "General";
//       } else {
//         locationName = firstLoc.toString();
//         locationName2 = secondLoc.toString() ;
//       }
//     } else if (locData is Map) {
//       // If it's a Map
//       locationName = locData['name']?.toString() ?? "General";
//       // locationName2 = locData['name']?.toString() ?? "General";
